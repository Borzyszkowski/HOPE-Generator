...
def build_optimizer(
    models: [nn.Module], optim_cfg: DictConfig, exclude: str = ""
...
def get_optimizer(
    params: TensorList, optim_cfg: DictConfig
) -> Union[optim.SGD, optim.Adam, optim.RMSprop]:
    lr = optim_cfg.lr
    optimizer_type = optim_cfg.type
    logger.info(f"Building optimizer: {optimizer_type.title()}")
    if optimizer_type == "sgd":
        optimizer = optim.SGD(params, lr, **OmegaConf.to_container(optim_cfg.sgd))
    elif optimizer_type == "adam":
        optimizer = optim.Adam(params, lr, **OmegaConf.to_container(optim_cfg.adam))
    elif optimizer_type == "adamw":
        optimizer = optim.AdamW(params, lr, **OmegaConf.to_container(optim_cfg.adam))
...
def build_scheduler(optimizer, sched_cfg):
    ...
    if scheduler_type == "none":
        ...
        logger.info(
            "Building scheduler: StepLR(step_size={}, gamma={})", step_size, gamma
        )
        ...
        logger.info(
            "Building scheduler: MultiStepLR(milestone={}, gamma={})", milestones, gamma
        )
        return scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=gamma)
    else:
