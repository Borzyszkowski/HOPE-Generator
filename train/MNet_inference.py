...
from tools.train_tools import EarlyStopping, WeightAnneal, v2v
from tools.utils import (LOGGER_DEFAULT_FORMAT, aa2rotmat, create_video,
                         d62rotmat, makelogger, makepath, rotmat2aa, to_cpu,
                         to_np, to_tensor)
...
class Trainer:
    def __init__(self, cfg, inference=False):
        ...
        if use_cuda:
            self.logger(
                "Using %d CUDA cores [%s] for training!" % (gpu_count, gpu_brand)
            )
        ...
        if inference and cfg.best_model is None:
            cfg.best_model = sorted(
                glob.glob(
                    os.path.join(cfg.work_dir, "snapshots", "*[0-9][0-9][0-9]_model.pt")
                )
            )[-1]
        if cfg.best_model is not None:
            self._get_network().load_state_dict(
    ...
    def loss_setup(self):
        ...
        self.logger(
            f"Vertex loss, weight: {self.vertex_loss}," f" {self.vertex_loss_weight}"
        )
        ...
        self.logger(
            f"Right Hand Vertex loss, weight: {self.rh_vertex_loss},"
        ...
        self.logger(
            f"Feet Vertex loss, weight: {self.feet_vertex_loss},"
        ...
        self.logger(f"Pose loss, weight: {self.pose_loss}," f" {self.pose_loss}")
        ...
        self.logger(
            f"Velocity loss, weight: {self.velocity_loss},"
        ...
        self.logger(f"Contact loss, weight: " f" {self.contact_loss_weight}")

        kl_loss_cfg = loss_cfg.get("kl_loss", {})
        self.kl_loss_weight = kl_loss_cfg.get("weight", 0.0)
        self.logger(f"KL loss, weight: " f" {self.kl_loss_weight}")

        rh_f = torch.from_numpy(np.load(loss_cfg.rh_faces).astype(np.int32)).view(
            1, -1, 3
        )
        self.rh_f = rh_f.repeat(self.cfg.datasets.batch_size, 1, 1).to(torch.long)

        self.verts_ids = to_tensor(
            np.load(self.cfg.datasets.verts_sampled), dtype=torch.long
        )
        self.feet_ids_sampled = to_tensor(
            np.load(self.cfg.datasets.verts_feet), dtype=torch.long
        )
    ...
    def train(self):
        ...
        for it, batch in enumerate(self.ds_train):
            ...
            if it % (save_every_it + 1) == 0:
                cur_train_loss_dict = {
                    k: v / (it + 1) for k, v in train_loss_dict.items()
                }
        ...
        train_loss_dict = {
            k: v / len(self.ds_train) for k, v in train_loss_dict.items()
        }

        return train_loss_dict

    def evaluate(self, ds_name="train"):
        self.network.eval()

        eval_loss_dict = {}
        data = self.ds_val if ds_name == "train" else self.ds_test
    ...
    def get_loss(self, batch, batch_idx, results):
        ...
        v_template = (
            batch["sbj_vtemp"]
            .reshape(B, 1, -1, 3)
            .expand(-1, self.n_out_frames, -1, -1)
        )
        ...
        bparams = {
            k: v.reshape([-1, self.n_out_frames] + list(v.shape[1:]))
        ...
        if FN > 0:
            ...
            f_params = {
                k: v[females].reshape([FN * self.n_out_frames] + list(v.shape[2:]))
                for k, v in bparams.items()
            }
            ...
            f_verts_xyz = (
                verts_offset[females].reshape([FN, -1, 400, 3]) * 0.01 + f_verts_init
            )
        ...
        if MN > 0:
            ...
            m_params = {
                k: v[males].reshape([MN * self.n_out_frames] + list(v.shape[2:]))
                for k, v in bparams.items()
            }
            ...
            m_verts_xyz = (
                verts_offset[males].reshape([MN, -1, 400, 3]) * 0.01 + m_verts_init
            )
        ...
        if self.vertex_loss_weight > 0:
            ...
            losses_w["vertices_xyz"] = losses["vertices_xyz"] * (
                self.vertex_loss_weight + 3
            )

        if self.pose_loss_weight > 0:
            losses["pose"] = self.LossL2(
                batch["fullpose_rotmat"][:, 11 : 11 + self.n_out_frames],
                bparams["fullpose_rotmat"],
            )
            losses["trans"] = self.LossL1(
                batch["transl"][:, 11 : 11 + self.n_out_frames], bparams["transl"]
            )
        ...
        verts2last = (
            batch["verts"][:, 10:11, self.rh_ids_sampled]
            - batch["verts"][:, -1:, self.rh_ids_sampled]
        )
        ...
        if self.rh_vertex_loss_weight > 0:
            ...
            if FN > 0:
                ...
                w = (rh2rh < 0.3).to(rh2rh.dtype) + torch.exp(-10 * rh2rh)
                w = w.reshape(-1, 1, 1, 1)
                losses["rh_vertices"] += self.vertex_loss(
                    w * f_verts_gt[:, :, self.rh_ids_sampled],
                    w * f_verts[:, :, self.rh_ids_sampled],
                )
                losses["rh_vertices_xyz"] += self.vertex_loss(
                    w * f_verts_gt[:, :, self.rh_ids_sampled],
            ...
            if MN > 0:
                ...
                w = w.reshape(-1, 1, 1, 1)
                losses["rh_vertices"] += self.vertex_loss(
                    w * m_verts_gt[:, :, self.rh_ids_sampled],
            ...
            losses_w["rh_vertices"] = (
                losses["rh_vertices"] * self.rh_vertex_loss_weight / 2
            )
            losses_w["rh_vertices_xyz"] = (
                losses["rh_vertices_xyz"] * self.rh_vertex_loss_weight
            )

        if self.contact_loss_weight > 0:
            dist_gt = (
                batch["verts"][:, 11 : 11 + self.n_out_frames] - batch["verts"][:, -1:]
            )[:, :, self.rh_ids_sampled]
            dist_init = verts2last
            dist_hat = (
                rh2last.reshape(dist_gt.shape) * 0.01 + dist_init
            )  ## get the dist offset in cm

            min_dist = dist_gt.norm(dim=-1).min(dim=-1)[0]
            w = (min_dist < 0.4).to(min_dist.dtype) + torch.exp(-10 * min_dist)
        ...
        if self.feet_vertex_loss_weight > 0:
            feet_verts_gt = batch["verts"][
                :, 11 : 11 + self.n_out_frames, self.feet_ids_sampled
            ]
            ...
            if FN > 0:
                ...
                losses["feet_vertices"] += self.vertex_loss(
                    w * f_verts_gt[:, :, self.feet_ids_sampled],
                    w * f_verts[:, :, self.feet_ids_sampled],
                )
                losses["feet_vertices_xyz"] += self.vertex_loss(
                    w * f_verts_gt[:, :, self.feet_ids_sampled],
            ...
            if MN > 0:
                ...
                w = 1 + torch.exp(-20 * feet2grnd)
                losses["feet_vertices"] += self.vertex_loss(
                    w * m_verts_gt[:, :, self.feet_ids_sampled],
            ...
            losses_w["feet_vertices"] = (
                losses["feet_vertices"] * self.feet_vertex_loss_weight / 2
            )
            losses_w["feet_vertices_xyz"] = (
                losses["feet_vertices_xyz"] * self.feet_vertex_loss_weight
            )
        ...
        with torch.no_grad():
            ...
            if FN > 0:
                ...
                loss_v2v_hands.append(
                    v2v(
                        f_verts_xyz[:, :, self.rh_ids_sampled],
                        f_verts_gt[:, :, self.rh_ids_sampled],
                        mean=False,
                    )
                )
                loss_v2v_feet.append(
                    v2v(
                        f_verts_xyz[:, :, self.feet_ids_sampled],
                        f_verts_gt[:, :, self.feet_ids_sampled],
                        mean=False,
                    )
                )
            if MN > 0:
                ...
                loss_v2v_hands.append(
                    v2v(
                        m_verts_xyz[:, :, self.rh_ids_sampled],
                        m_verts_gt[:, :, self.rh_ids_sampled],
                        mean=False,
                    )
                )
                loss_v2v_feet.append(
                    v2v(
                        m_verts_xyz[:, :, self.feet_ids_sampled],
                        m_verts_gt[:, :, self.feet_ids_sampled],
                        mean=False,
                    )
                )
    ...
    def set_weight_annealing(self):
        ...
        self.vertex_loss_weight_ann = WeightAnneal(
            start_w=self.vertex_loss_weight / 4,
            end_w=self.vertex_loss_weight,
            start_batch=0,
            end_batch=4,
        )
        self.pose_loss_weight_ann = WeightAnneal(
            start_w=self.pose_loss_weight,
            end_w=self.pose_loss_weight / 4,
            start_batch=0,
            end_batch=4,
        )

        self.contact_loss_weight_ann = WeightAnneal(
            start_w=self.contact_loss_weight / 4,
            end_w=self.contact_loss_weight,
            start_batch=0,
            end_batch=4,
        )

        self.rh_vertex_loss_weight_ann = WeightAnneal(
            start_w=self.rh_vertex_loss_weight / 10,
            end_w=self.rh_vertex_loss_weight,
            start_batch=4,
            end_batch=10,
        )
        self.feet_vertex_loss_weight_ann = WeightAnneal(
            start_w=self.feet_vertex_loss_weight / 10,
            end_w=self.feet_vertex_loss_weight,
            start_batch=4,
            end_batch=10,
        )
    ...
    def set_loss_weights(self):
        ...
        self.rh_vertex_loss_weight = self.rh_vertex_loss_weight_ann(
            self.epochs_completed
        )
        self.feet_vertex_loss_weight = self.feet_vertex_loss_weight_ann(
            self.epochs_completed
        )
                ...
                if eval_loss_dict["loss_v2v"] < self.best_loss:
                    ...
                    self.cfg.best_model = makepath(
                        os.path.join(
                            self.cfg.work_dir,
                            "snapshots",
                            "E%03d_model.pt" % (self.epochs_completed),
                        ),
                        isfile=True,
                    )
                    self.save_network()
    ...
    def configure_optimizers(self):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, "min", factor=0.2, patience=8
        )
        self.early_stopping = EarlyStopping(
            **self.cfg.network.early_stopping, trace_func=self.logger
        )
    ...
    def inference_generate(self):
        ...
        for batch_id, batch in enumerate(data):
            ...
            movie_name = "s" + self.data_info[ds_name]["frame_names"][
                batch["idx"].to(torch.long)
            ][0][:-2].split("/s")[-1].replace("/", "_")
            mov_count = 1
            movie_path = os.path.join(
                base_movie_path, str(mov_count), movie_name + ".html"
            )
            grasp_meshes_path = os.path.join(
                self.cfg.results_base_dir, f"{obj_name}_grasp"
            )
            ...
            while moving_b.num_iters < 10:
                ...
                min_dist_offset = (
                    net_output["rh2last"].reshape(-1, 3).norm(dim=-1).max()
                )
...
def inference():
    ...
    parser.add_argument("--expr-id", default=None, type=str, help="Training ID")
