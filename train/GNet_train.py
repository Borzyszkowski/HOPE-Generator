...
class Trainer:
    ...
    def forward(self, x):
        ...
        #####################################################
        enc_x = torch.cat(
            [v.reshape(bs, -1).to(self.device) for v in enc_x.values()], dim=1
        )
    ...
    def infer(self, x):
        ...
        z_enc = torch.distributions.normal.Normal(
            loc=torch.zeros(
                [1, self.cfg.network.gnet_model.latentD], requires_grad=False
            )
            .to(self.device)
            .type(self.dtype),
            scale=torch.ones(
    ...
    def get_loss(self, batch, batch_idx, results):
        ...
        params_gt = parms_6D2full(
            batch["fullpose_rotmat"], batch["transl"], d62rot=False
        )
        if FN > 0:
        ...
        if self.vertex_loss_weight > 0:
            ...
            if FN > 0:
                losses["cnet_vertices"] += self.vertex_loss(
                    f_verts_gt, cnet["f_verts_full"]
                )
            if MN > 0:
                losses["cnet_vertices"] += self.vertex_loss(
                    m_verts_gt, cnet["m_verts_full"]
                )
        ...
        if self.pose_loss_weight > 0:
            ...
            losses["cnet_trans"] = 0
            if FN > 0:
                losses["cnet_pose"] += self.LossL2(
                    f_params_gt["fullpose_rotmat"], cnet["f_params"]["fullpose_rotmat"]
                )
                losses["cnet_trans"] += self.LossL1(
                    f_params_gt["transl"], cnet["f_params"]["transl"]
                )
            if MN > 0:
                losses["cnet_pose"] += self.LossL2(
                    m_params_gt["fullpose_rotmat"], cnet["m_params"]["fullpose_rotmat"]
                )
                losses["cnet_trans"] += self.LossL1(
                    m_params_gt["transl"], cnet["m_params"]["transl"]
                )
        ...
        if self.rh_vertex_loss_weight > 0:
            ...
            if FN > 0:
                losses["cnet_rh_vertices"] += self.vertex_loss(
                    f_verts_gt[:, self.rhand_idx],
                    cnet["f_verts_full"][:, self.rhand_idx],
                )
            if MN > 0:
                losses["cnet_rh_vertices"] += self.vertex_loss(
                    m_verts_gt[:, self.rhand_idx],
                    cnet["m_verts_full"][:, self.rhand_idx],
                )
        ...
        losses["rh2obj"] = self.vertex_loss_weight * self.LossL1(
            rh2obj_w * rh2obj_gt, rh2obj_w * cnet["dist"].reshape(rh2obj_gt.shape)
        )
        ...
        e_z = torch.distributions.normal.Normal(
            enc_z["mean"], enc_z["std"]
        )  # encoder distribution

        n_z = torch.distributions.normal.Normal(
            loc=torch.zeros(
                [self.cfg.datasets.batch_size, self.cfg.network.gnet_model.latentD],
                requires_grad=False,
            )
            .to(self.device)
            .type(self.dtype),
        ...
        losses["loss_kl_encoder"] = self.kl_loss_weight * torch.mean(
            torch.sum(torch.distributions.kl.kl_divergence(e_z, n_z), dim=[1])
        ...
        with torch.no_grad():
            ...
            if FN > 0:
                loss_v2v.append(v2v(f_verts_gt, cnet["f_verts_full"], mean=False))
            if MN > 0:
                loss_v2v.append(v2v(m_verts_gt, cnet["m_verts_full"], mean=False))
    ...
    def inference_generate(self):
        ...
        for batch_id, batch in enumerate(data):
            ...
            movie_name = movie_name[
                : np.where([not i.isdigit() for i in movie_name])[0][-1]
            ]
            # if batch_id%100 !=0 :
            ...
            for i in range(num_samples):
                ...
                sp_anim.add_frame(
                    ...
                    ["course_grasp", "refined_grasp", "object", "ground_mesh"],
                )
            ############################
...
def train():
    ...
    parser.add_argument("--expr-id", default="GNet_V00", type=str, help="Training ID")
    ...
    cfg.datasets.dataset_dir = os.path.join(cmd_args.data_path, "GNet_data")
