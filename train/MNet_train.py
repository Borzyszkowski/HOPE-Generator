...
class Trainer:
    def __init__(self, cfg, inference=False):
        ...
        logger_path = makepath(
            os.path.join(
                cfg.work_dir,
                "%s_%s.log" % (cfg.expr_ID, "train" if not inference else "test"),
            ),
            isfile=True,
        )
        ...
        self.logger(
            "[%s] - Started training XXX, experiment code %s" % (cfg.expr_ID, starttime)
        )
    ...
    def load_data(self, cfg, inference):
        ...
        self.data_info[ds_name]["chunk_starts"] = (
            np.array(
                [
                    int(fname.split("_")[-1])
                    for fname in self.data_info[ds_name]["frame_names"][:, 1]
                ]
            )
            == 0
        )
...
def train():
    ...
    parser.add_argument("--expr-id", default="MNet_V00", type=str, help="Training ID")
    ...
    parser.add_argument(
        "--num-gpus", default=1, type=int, help="Number of multiple GPUs for training"
    )
...
def run_trainer_once(cfg):
    ...
    OmegaConf.save(
        trainer.cfg, os.path.join(cfg.work_dir, "{}.yaml".format(cfg.expr_ID))
    )
