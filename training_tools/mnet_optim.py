...
class MNetOpt(nn.Module):
    def __init__(self, sbj_model, obj_model, cfg, verbose=False):
        ...
        self.rhand_idx = (
            to_tensor(np.load(f"{cdir}/{self.cfg.losses.rh2smplx_idx}"))
            .to(torch.long)
            .to(self.device)
        )
        self.rh_ids_sampled = (
            to_tensor(np.where([id in self.rhand_idx for id in self.verts_ids])[0])
            .to(torch.long)
            .to(self.device)
        )
        self.feet_ids_sampled = to_tensor(
            np.load(f"{cdir}/{self.cfg.datasets.verts_feet}"), dtype=torch.long
        ).to(self.device)
    ...
    def config_optimizers(self):
        ...
        self.opt_params = {
            "global_orient": torch.randn(
                bs, 1 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "body_pose": torch.randn(
                bs, 21 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "left_hand_pose": torch.randn(
                bs, 15 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "right_hand_pose": torch.randn(
                bs, 15 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "jaw_pose": torch.randn(
                bs, 1 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "leye_pose": torch.randn(
                bs, 1 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "reye_pose": torch.randn(
                bs, 1 * 3, device=device, dtype=dtype, requires_grad=True
            ),
            "transl": torch.zeros(
                bs, 3, device=device, dtype=dtype, requires_grad=True
            ),
        }
        ...
        self.opt_s3 = optim.Adam(
            [self.opt_params[k] for k in ["transl", "body_pose", "right_hand_pose"]],
            lr=lr,
        )
    ...
    def init_params(self, start_params):
        ...
        fullpose_aa = rotmat2aa(start_params["fullpose_rotmat"]).reshape(
            self.n_out_frames, -1
        )
    ...
    def get_smplx_verts(self, batch, output):
        ...
        self.rh2rh_net = (
            ...
            - self.net_verts[:, self.rh_ids_sampled]
        ).clone()
    ...
    def get_weights(self):
        ...
        if is_close.any():
            ...
            idx2last_dist = (
                self.rh2rh_net[idx_match - 1 : idx_match].mean(dim=1).norm(dim=-1)
            )
            ...
            if rh_net_vel_m.max() < 0.02:
                ...
                rh_net_vel_m = torch.stack(
                    [
                        rh_net_vel_m[1] - i * delta * rh_net_vel_m[1]
                        for i in range(1, 11)
                    ],
                    dim=0,
                )
    ...
    def remove_lasts(self, opt_output):
        ...
        if ones.any():
            ...
            for k, v in self.opt_params.items():
                self.opt_params[k].data[ids:] = torch.repeat_interleave(
                    self.opt_params[k].data[ids : ids + 1], ones.sum(), 0
                )

            fullpose_rotmat[ids:] = torch.repeat_interleave(
                fullpose_rotmat[ids : ids + 1], ones.sum(), 0
            )

        body_params = {
            k: aa2rotmat(v.detach())
            for k, v in self.opt_params.items()
            if v != "transl"
        }
    ...
    def calc_loss(self, batch, net_output, stage):
        ...
        opt_params = {
            k: aa2rotmat(v) for k, v in self.opt_params.items() if k != "transl"
        }
        ...
        # # verts_loss_w = 1
        # #
        rh2obj = self.bps_torch.encode(
            x=torch.repeat_interleave(self.obj_verts, self.bs, dim=0),
            feature_type=["deltas"],
            custom_basis=verts[:, self.verts_ids[self.rh_ids_sampled]],
        )["deltas"]

        rh2obj_last = (
            batch["verts2obj"][:, -1:]
            .reshape(1, -1, 3)
            .repeat(self.bs, 1, 1)[:, self.rh_ids_sampled]
        )

        grasp_rh_pose = torch.repeat_interleave(
            rotmat2aa(batch["fullpose_rotmat"][:, -1:, 40:]).reshape(1, -1),
            self.n_out_frames,
            dim=0,
        ).reshape(-1)
        ...
        linear_rh2rh = (
            self.net_verts[self.idx_match - 1 : self.idx_match, self.rh_ids_sampled]
            + self.weights * self.rh2rh_net[self.idx_match - 1 : self.idx_match]
        )

        losses["linear_rh2rh"] = 20 * self.LossL1(
            rh_verts_opt[self.idx_opt :], linear_rh2rh
        )
        # losses['rh2rh_offset'] = 5 * self.LossL1(rh2rh_opt, rh2rh_net)
        losses["rh_grasp_pose"] = 0.1 * self.LossL2(
            torch.exp(-5 * self.weights) * grasp_rh_pose.reshape(self.bs, -1).detach(),
            torch.exp(-5 * self.weights)
            * self.opt_params["right_hand_pose"].reshape(self.bs, -1),
        )
        losses["rh2obj"] = 5 * self.LossL1(
            (self.weights == 1) * rh2obj[self.idx_opt :],
            (self.weights == 1) * rh2obj_last[self.idx_opt :],
        )
        ...
        body_loss = {
            ...
            * pose_w
            * self.LossL2(
                rotmat2aa(self.sbj_params[k]).detach().reshape(-1),
                self.opt_params[k].reshape(-1),
            )
            for k, w in [
                ("global_orient", 1),
                ("body_pose", 0.5),
                ("left_hand_pose", 1),
                ("right_hand_pose", 0.02),
            ]
        }
        ...
        body_loss["transl"] = (
            100
            * pose_w
            * self.LossL1(self.opt_params["transl"], self.sbj_params["transl"].detach())
        )
