...
class ObjectModel(nn.Module):
    def __init__(self, v_template=None, batch_size=1, dtype=torch.float32):
    ...
    def forward(
        self, global_orient=None, transl=None, v_template=None, pose2rot=True, **kwargs
    ):
        ...
        if global_orient is None:
            pose2rot = False
            global_orient = batch_rodrigues(self.global_orient.view(-1, 3)).reshape(
                [batch_size, 3, 3]
            )
        if transl is None:
        ...
        if pose2rot:
            global_orient = batch_rodrigues(global_orient.view(-1, 3)).reshape(
                [batch_size, 3, 3]
            )
