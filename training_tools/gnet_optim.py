""" Optimization (pose refinement) for GNet model """
...
class GNetOptim(nn.Module):
    def __init__(self, sbj_model, obj_model, cfg, verbose=False):
        ...
        self.rhand_idx = to_tensor(
            np.load(f"{cdir}/{self.cfg.losses.rh2smplx_idx}"), dtype=torch.long
        )
        self.rhand_tri = to_tensor(
            np.load(f"{cdir}/{self.cfg.losses.rh_faces}").astype(np.int32)
        )
    ...
    def config_optimizers(self):
        ...
        self.opt_s3 = optim.Adam(
            [
                self.opt_params[k]
                for k in ["global_orient", "transl", "body_pose", "right_hand_pose"]
            ],
            lr=lr,
        )
    ...
    def init_params(self, start_params):
        ...
        for k in self.opt_params.keys():
            self.opt_params[k].data = torch.repeat_interleave(
                start_params_aa[k], self.bs, dim=0
            )
    ...
    def calc_loss(self, batch, net_output, stage):
        ...
        losses = {
            ...
            "grnd_contact": (verts[:, :, 1].min() < -0.02)
            * torch.pow(verts[:, :, 1].min() + 0.01, 2),
        ...
        k = "right_hand_pose"
        body_loss[k] = 0.3 * self.LossL2(
            rotmat2aa(self.sbj_params[k]).detach().reshape(-1),
            self.opt_params[k].reshape(-1),
        )
        body_loss["transl"] = self.LossL1(
            self.opt_params["transl"], self.sbj_params["transl"].detach()
        )
    ...
    def get_peneteration(self, source_mesh, target_mesh):
        ...
        src2trgt, trgt2src, src2trgt_idx, trgt2src_idx = self.ch_dist(
            source_verts.reshape(1, -1, 3).to(self.device),
        ...
        in_out = (
            torch.bmm(source_normals.view(-1, 1, 3), distance_vector.view(-1, 3, 1))
            .view(-1)
            .sign()
        )
