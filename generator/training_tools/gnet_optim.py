...
class GNetOptim(nn.Module):
    ...
    def __init__(self,
        ...
        self.rhand_idx = to_tensor(np.load(f'{cdir}/{self.cfg.losses.rh2smplx_idx}'), dtype=torch.long)
        self.rhand_tri = to_tensor(np.load(f'{cdir}/{self.cfg.losses.rh_faces}').astype(np.int32))
