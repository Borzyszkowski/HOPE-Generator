...
class MNetDataSet(object):
    ...
    def data_preprocessing(self, cfg):
        ...
        for split in self.split_seqs.keys():
            ...
            for seq_i, sequence in enumerate(tqdm(self.split_seqs[split])):
                ...
                obj_info = self.load_obj_verts(obj_name, cfg.n_verts_sample)
                ...
                with torch.no_grad():
                    ...
                    oi_shape = OakInkShape(category="teapot", intent_mode="use", data_split=split)
                    ...
                    for ch_id, ch in enumerate(chunks):
                        ...
                        motion_sbj, motion_obj, rel_trans = glob2rel(sbj_params, obj_params, R_inv, root_offset, wind,
                                                                     past)
                        ...
                        obj_in = {k + '_obj': to_cpu(v.reshape([1, wind] + list(v.shape[1:]))) for k, v in
                                  motion_obj.items()}
    ...
    def load_obj_verts(self, obj_name, n_verts_sample=2048):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out-path',
                        default="_DATA/MNet_data_OakInk/",
                        type=str,
