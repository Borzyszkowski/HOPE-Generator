...
def convert(id, depth_path, mask_path, output_background_path):
    ...
    depths, labels, labels_instanceseg = get_foreground_depths_and_labels(
        depth_path, mask_path, ds, ls, ls_instanceseg
    )
    for i in range(len(depths)):
        output_foreground_p = os.path.join(
            output_background_path.replace("background", "foreground_" + str(i + 1)),
            id + ".png",
        )
...
def scene2frame(
    ...
    for filename in filelist:
        ...
        depth_foregrounds, depth_background = convert(
            id,
            os.path.join(depth_path, id + ".png"),
            os.path.join(mask_path, id.zfill(5) + ".png"),
            output_background_path,
        )
        ...
        pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
            ...
            extrinsic=extrinsics[int(id)],
        )
        # pcd.transform(extrinsics[int(id)])
        voxel_down_pcd = pcd.voxel_down_sample(voxel_size=0.02)
        o3d.io.write_point_cloud(
            os.path.join(output_label_path, id + ".ply"), voxel_down_pcd
        )
        ...
        data_np_0 = np.zeros(
            (data_pd_0.shape[0], data_pd_0.shape[1] + 2), dtype=np.float64
        )  # 初始化储存数据的array
        property_names_0 = data_0[0].dtype.names  # 读取property的名字
        for i, name in enumerate(
            property_names_0
        ):  # 按property读取数据，这样可以保证读出的数据是同样的数据类型。
        ...
        for i in range(len(depth_foregrounds)):
            ...
            if points.shape[0] == 0:
                continue
            o3d.io.write_point_cloud(
                os.path.join(output_label_path, id + "_f_" + str(i) + ".ply"),
                voxel_down_pcd,
            )

            plydata_0 = PlyData.read(
                os.path.join(output_label_path, id + "_f_" + str(i) + ".ply")
            )
            data_0 = plydata_0.elements[0].data
            data_pd_0 = pd.DataFrame(data_0)
