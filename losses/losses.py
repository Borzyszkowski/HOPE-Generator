    ...
    def forward(
        ...
        if mask is None:
            if weight is None:
                ...
                return torch.einsum("ijk, ij -> ijk", diff.pow(2), weight).sum() / (
                    diff.shape[0] + self.epsilon
                )
        else:
            if weight is None:
                ...
                return (
                    mask * torch.einsum("ijk, ij -> ijk", diff.pow(2), weight)
                ).sum() / mask.sum().clamp_(min=self.epsilon)


class RotationDistance(nn.Module):
    def __init__(self, reduction="mean", epsilon=1e-7, robustifier="none", **kwargs):
    ...
    def forward(self, module_input, target, weights=None):
        tr = torch.einsum(
            "bij,bij->b", [module_input.view(-1, 3, 3), target.view(-1, 3, 3)]
        )

        theta = (tr - 1) * 0.5
        loss = torch.acos(torch.clamp(theta, -1 + self.epsilon, 1 - self.epsilon))
        if weights is not None:
            loss = loss.view(module_input.shape[0], -1) * weights.view(
                module_input.shape[0], -1
            )
            return loss.sum() / (weights.gt(0).to(loss.dtype).sum() + self.epsilon)
        else:
...
class VertexEdgeLoss(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.has_connections = osp.exists(gt_edge_path) and osp.exists(est_edge_path)
    ...
    def compute_edges(
        ...
        start = torch.index_select(points, vertex_axis, connections[:, 0])
    ...
    def forward(
        ...
        gt_edges = self.compute_edges(
            gt_vertices, vertex_axis=vertex_axis, connections=self.gt_connections
        )
        est_edges = self.compute_edges(
            est_vertices, vertex_axis=vertex_axis, connections=self.est_connections
        )
