...
DEFAULT_NUM_WORKERS = {"train": 0, "val": 0, "test": 0}
...
class LoadData(data.Dataset):
    def __init__(self, cfg, split_name="train"):
        ...
        self.frame_sbjs = np.asarray(
            [name.split("/")[-2] for name in self.frame_names[:, 10]]
        )
        self.frame_st_end = np.asarray(
            [int(name.split("_")[-1]) for name in self.frame_names[:, 10]]
        )
        self.frame_objs = np.asarray(
            [os.path.basename(name).split("_")[0] for name in self.frame_names[:, 10]]
        )

        self.obj_info = np.load(
            os.path.join(dataset_dir, "obj_info.npy"), allow_pickle=True
        ).item()
        self.sbj_info = np.load(
            os.path.join(dataset_dir, "sbj_info.npy"), allow_pickle=True
        ).item()
        ...
        self.sbj_vtemp = torch.from_numpy(
            np.asarray([Mesh(filename=file).v.astype(np.float32) for file in file_list])
        )
        self.sbj_betas = torch.from_numpy(
            np.asarray(
                [
                    np.load(file=f.replace(".ply", "_betas.npy")).astype(np.float32)
                    for f in file_list
                ]
            )
        )
        ...
        self.frame_sbjs = torch.from_numpy(self.frame_sbjs.astype(np.int8)).to(
            torch.long
        )
        ...
        self.ds["gender"] = torch.Tensor(
            [
                1 if self.genders[sbj_id] == "male" else 2
                for sbj_id in self.ds["frame_sbj_ids"]
            ]
        ).to(torch.long)
        ...
        self.obj_verts = torch.from_numpy(
            np.asarray(
                [
                    self.obj_info[obj]["verts_sample"].astype(np.float32)
        ...
        self.frame_objs = torch.from_numpy(self.frame_objs.astype(np.int8)).to(
            torch.long
        )
    ...
    def normalize(self):
        ...
        if os.path.exists(norm_data_dir):
            ...
            in_p = {
                k: (v.mean(0, keepdim=True), v.std(0, keepdim=True) + 1e-10)
                for k, v in self.ds["in"].items()
                if v.dtype == torch.float
            }
            out_p = {
                k: (v.mean(0, keepdim=True), v.std(0, keepdim=True) + 1e-10)
                for k, v in self.ds["out"].items()
            }
    ...
    def load_idx(self, idx, source=None):
        ...
        obj_m = ObjectModel(v_template=idx_obj, batch_size=bs)
