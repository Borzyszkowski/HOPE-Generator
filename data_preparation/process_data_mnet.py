...
class MNetDataSet(object):
    ...
    def data_preprocessing(self, cfg):
        ...
        for split in self.split_seqs.keys():
            # split = 'train'
            outfname = makepath(
                os.path.join(cfg.out_path, split, "grasp_motion_data.npy"), isfile=True
            )
            ...
            for seq_i, sequence in enumerate(tqdm(self.split_seqs[split])):
                ...
                with torch.no_grad():
                    ...
                    frames = torch.cat(
                        [torch.zeros(past), frames, torch.ones(future) * (T - 1)]
                    ).to(torch.long)
                    chunks = frames.unfold(
                        dimension=0, size=wind - 1, step=1
                    )  # create motion chuncks

                    for ch_id, ch in enumerate(chunks):

                        ch = torch.cat([ch, torch.tensor([-1])]).to(
                            torch.long
                        )  # to get last frame of motion
                        ...
                        theta = torch.acos(torch.einsum("ij,ij->i", z_w, z_s)).reshape(
                            -1, 1
                        )
                        ...
                        motion_sbj, motion_obj, rel_trans = glob2rel(
                            sbj_params, obj_params, R_inv, root_offset, wind, past
                        )
                        ...
                        sbj_in = {
                            k: to_cpu(v.reshape([1, wind] + list(v.shape[1:])))
                            for k, v in motion_sbj.items()
                        }
                        obj_in = {
                            k + "_obj": to_cpu(v.reshape([1, wind] + list(v.shape[1:])))
                        ...
                        grasp_motion_data["verts"].append(
                            to_cpu(verts_sbj[:, verts_ids].reshape(1, wind, -1, 3))
                        )
                        grasp_motion_data["rel_rot"].append(
                            to_cpu(R_inv.reshape(1, 3, 3))
                        )
                        grasp_motion_data["rel_trans"].append(
                            to_cpu(rel_trans.reshape(1, 3))
                        )
                        ...
                        grasp_motion_data["bps_obj_glob"].append(
                            to_cpu(bps_obj).reshape(1, 1, -1)
                        )

                        bps_rh = self.bps_torch.encode(
                            x=verts_sbj[-1:, rh_verts_ids],
                            feature_type=["deltas"],
                            custom_basis=obj_bps,
                        )["deltas"]

                        grasp_motion_data["bps_rh_glob"].append(
                            to_cpu(bps_rh).reshape(1, 1, -1)
                        )

                        grasp_motion_data["full_seq_id"].append(
                            np.array([seq_i]).reshape(1, -1).astype(int)
                        )

                        frame_names.extend(
                            [
                                "%s_%s" % (sequence.split(".")[0], fId)
                                for fId in to_cpu(chunks[ch_id])
                            ]
                        )
    ...
    def filter_grasp_frames(self, seq_data):
        ...
        hand_ang_vel = (
            loc2vel(hand_rotmat, fps=120).abs().norm(dim=-1).norm(dim=-1).squeeze()
        )
        ...
        if fil.sum() < 1:
            return fil
        skip_frame = int(120.0 / self.cfg.fps)
        ...
        grasp_motion_frames = (
            grasp_frames * include_fil * exclude_fil * fps_fil * start_fil
        )
